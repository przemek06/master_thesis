...
def eval_series(model, u, y):
    ...
    target = denormalize(y[WARMUP:WARMUP + MAX_ROLLOUT])
    chunks = []
    n = 0
    while n < MAX_ROLLOUT:
        chunks.append(model.predict_autonomous(min(CHUNK, MAX_ROLLOUT - n)))
        pred = denormalize(np.concatenate(chunks))
        n = len(pred)
...
def run_one(task):
    ...
    u_train, y_train, val_list, test_list = load(
        n_val=N_VAL, n_test=N_TEST, n_train=N_TRAIN, seed_offset=seed_offset
    )
...
def main():
    ...
    with open(os.path.join(HERE, "results.txt"), "w") as f:
        f.write("=== Mackey-Glass, 10 runs with per-run data ===\n")
        for m in MODELS:
            s = results["summary"][m]
            f.write(f"{m}:\n")
