...
import optuna
from tasks.mackey_glass import load
from models.custom_esn_4 import CustomESN4
...
Q = generate_isospectral_sparse_orthogonal_matrix(
    lambda size: sample_eigenvalues_ginibre(r_min=0.1, r_max=0.99, alpha=2.0, size=size),
    N_RESERVOIR,
    SPARSITY,
...
print(f"Q sparsity: {np.mean(Q == 0):.4f}")
...
def objective(trial):
    ...
    model = CustomESN4(n_inputs=1, n_reservoir=N_RESERVOIR, n_outputs=1, Q=Q, seed=0, **params)
...
model = CustomESN4(n_inputs=1, n_reservoir=N_RESERVOIR, n_outputs=1, Q=Q, seed=0, **best)
model.fit(u_train, y_train, warmup=WARMUP)
...
fig.suptitle("CustomESN4 — Mackey-Glass predictions vs target")
plt.tight_layout()
...
plt.xlabel("Trial")
plt.ylabel("Val NMSE")
plt.title("Hyperparameter optimization — CustomESN4 Mackey-Glass")
plt.legend()
plt.tight_layout()
...
ax.set_aspect("equal")
ax.set_title("Eigenvalues of W — CustomESN4 Mackey-Glass")
ax.set_xlabel("Re")
ax.set_ylabel("Im")
plt.tight_layout()
