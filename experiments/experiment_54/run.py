...
for run_idx in range(N_RUNS):
    ...
    print(f"  ESNFeedback val MC: {np.mean(val_totals):.2f}, test MC: {np.mean(test_totals):.2f}", flush=True)
...
with open(os.path.join(HERE, "results.txt"), "w") as f:
    ...
    f.write(f"ESNFeedback test MC mean of means: {run_means.mean():.2f} ± {run_means.std():.2f} (bound: {N_RESERVOIR})\n")
    f.write("\n=== Per-run results ===\n")
...
for curve in run_test_curves:
    ax.plot(delays, curve, color="steelblue", alpha=0.25, linewidth=1)
ax.plot(delays, np.mean(run_test_curves, axis=0), color="steelblue", linewidth=2.5, label="Mean over runs")
ax.set_xlabel("Delay k")
ax.set_ylabel("MC_k")
ax.set_title(f"ESNFeedback — Forgetting curve (test), total MC = {run_means.mean():.2f} ± {run_means.std():.2f}")
ax.legend()
plt.tight_layout()
...
for ax, k in zip(axes.flat, [1, 5, 10, 25, 50, 100]):
    ...
    ax.scatter(target[:, k - 1], pred[:, k - 1], s=4, alpha=0.3, color="steelblue")
    ax.plot([-0.8, 0.8], [-0.8, 0.8], color="black", linewidth=1)
    ax.set_title(f"k={k}, MC_k={mc_k:.3f}")
    ax.set_xlabel("target u(t-k)")
    ax.set_ylabel("predicted")
fig.suptitle("ESNFeedback — Target vs predicted (last run, first test series)")
plt.tight_layout()
