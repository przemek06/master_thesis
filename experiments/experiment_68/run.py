...
def main():
    ...
    with open(os.path.join(HERE, "results.txt"), "w") as f:
        f.write(f"=== ESNFeedback | jena horizon={HORIZON} ===\n")
    ...
    ax.set_ylabel("Standardized T (degC)")
    ax.set_title(f"ESNFeedback — Jena {HORIZON}-step forecast (test NMSE={test_score:.4f})")
    ax.legend()
    plt.tight_layout()
