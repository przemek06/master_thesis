...
for m in MODELS:
    target = arrays[f"target_{m}"][WARMUP:]
    pred = arrays[f"pred_{m}"][WARMUP:]
    steps = int(arrays[f"steps_{m}"])
    end = min(steps + AFTER, len(pred))
    x = np.arange(end)
    fig, ax = plt.subplots(figsize=WIDE)
    ax.plot(x, target[:end], color=REFERENCE, linewidth=1)
    ax.plot(x, pred[:end], color=COLORS[m], linestyle="--")
    ax.axvline(steps, color=REFERENCE, linestyle="--", linewidth=1)
    ax.set_xlabel("step")
    ax.set_ylabel("value")
...
save(fig, "test_steps_bar.png")

lo = min(run_means[m].min() for m in MODELS) - 50
hi = max(run_means[m].max() for m in MODELS) + 50
