...
W = generate_isospectral_sparse_matrix(
    lambda size: sample_eigenvalues_ginibre(r_min=0.8, r_max=0.9, alpha=1.0, size=size),
    N_RESERVOIR,
    SPARSITY,
    iterations=50,
    seed=SEED,
...
val_auto_pred = denormalize(model.predict_autonomous(len(u_val) - WARMUP))
val_steps = steps_until_divergence(val_auto_pred, denormalize(y_val[WARMUP:]))
...
print(f"Val steps until divergence: {val_steps}")
print(f"Test NMSE: {test_nmse:.6f}")
print(f"Test steps until divergence: {test_steps}")

with open(os.path.join(HERE, "results.txt"), "w") as f:
    f.write(f"Val steps until divergence: {val_steps}\n")
...
ax.set_title(f"ESNCustomizable (Stiefel W) — Mackey-Glass refined (warmup={WARMUP}, val steps={val_steps}, test steps={test_steps})")
