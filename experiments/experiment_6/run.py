...
from models.custom_esn_1 import CustomESN1
...
def objective(trial):
    params = {
        ...
        "mu":            trial.suggest_float("mu", 0.0, np.pi),
        "kappa":         trial.suggest_float("kappa", 0.0, 10.0),
        "sparsity":      trial.suggest_float("sparsity", 0.9, 0.999),
        "input_scaling": trial.suggest_float("input_scaling", 0.1, 2.0),
        "ridge":         trial.suggest_float("ridge", 1e-9, 1e-1, log=True),
    }
    model = CustomESN1(n_inputs=1, n_reservoir=N_RESERVOIR, n_outputs=1, seed=0, **params)
...
model = CustomESN1(n_inputs=1, n_reservoir=N_RESERVOIR, n_outputs=1, seed=0, **best)
model.fit(u_train, y_train, warmup=100)
...
fig.suptitle("CustomESN1 (tuned) — NARMA-30 predictions vs target")
plt.tight_layout()
...
plt.xlabel("Trial")
plt.ylabel("Val NMSE")
plt.title("Hyperparameter optimization — CustomESN1 NARMA-30")
plt.legend()
plt.tight_layout()
