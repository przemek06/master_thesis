...
print(f"Sparsity mask density: {np.mean(sparsity_mask):.4f}")
...
def objective(trial):
    ...
    model = CustomESN5(n_inputs=1, n_reservoir=N_RESERVOIR, n_outputs=1,
                       T_template=T_template, Z_template=Z_template, sparsity_mask=sparsity_mask,
                       n_refine=20, seed=0, **params)
...
fig.suptitle("CustomESN5 — Mackey-Glass predictions vs target")
plt.tight_layout()
...
plt.xlabel("Trial")
plt.ylabel("Val NMSE")
plt.title("Hyperparameter optimization — CustomESN5 Mackey-Glass")
plt.legend()
plt.tight_layout()
...
ax.set_aspect("equal")
ax.set_title("Eigenvalues of W — CustomESN5 Mackey-Glass")
ax.set_xlabel("Re")
ax.set_ylabel("Im")
plt.tight_layout()
