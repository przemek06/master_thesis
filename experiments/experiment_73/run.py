...
def main():
    ...
    results = {
        "config": {
            ...
            "optuna_seeds": SEEDS,
            "note": "reservoir and data fixed; only the optuna (TPE) seed varies -> variance of the full tuning process. "
    ...
    ax.axhline(persistence, color="red", linestyle="--", linewidth=1, label=f"Persistence = {persistence:.4f}")
    ax.set_ylabel("Test NMSE")
    ax.set_title(f"Test NMSE across {N_REPLICATES} optuna seeds (jena)\nWelch p={p_value:.4f}")
    ax.legend()
    plt.tight_layout()
