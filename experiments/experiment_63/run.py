...
def main():
    ...
    with open(os.path.join(HERE, "results.txt"), "w") as f:
        f.write(f"=== ESNCustomizable (Ginibre) | ett {FILE} horizon={HORIZON} ===\n")
    ...
    ax.set_title(f"ESNCustomizable (Ginibre) — ETT {FILE} {HORIZON}-step forecast (test NMSE={test_score:.4f})")
    ax.legend()
    plt.tight_layout()
