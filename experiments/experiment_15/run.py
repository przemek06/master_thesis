...
for i, ax in enumerate(axes):
    if i == 0:
        ...
        full_true = np.concatenate([y_test[:WARMUP], y_test[WARMUP:WARMUP + len(y_pred)]])
        ax.axvspan(0, WARMUP, alpha=0.15, color="gray", label="Warmup")
        ax.plot(full_true[:WARMUP + 200], label="Target", color="steelblue")
...
fig.suptitle("ESN — Mackey-Glass predictions vs target")
plt.tight_layout()
...
plt.xlabel("Trial")
plt.ylabel("Val NMSE")
plt.title("Hyperparameter optimization — ESN Mackey-Glass")
plt.legend()
plt.tight_layout()
...
ax.set_aspect("equal")
ax.set_title("Eigenvalues of W — ESN Mackey-Glass")
ax.set_xlabel("Re")
ax.set_ylabel("Im")
plt.tight_layout()
