...
def build(params):
    return ESNFeedback(
        ...
        ridge=params["ridge"],
        noise=params["noise"],
        feedback_scaling=params["feedback_scaling"],
        seed=SEED,
        device="cpu",
    )
...
def main():
    ...
    results = {
        "config": {
            "model": "ESNFeedback (output feedback)",
            "task": "ett",
            "file": FILE,
            "horizon": HORIZON,
    ...
    with open(os.path.join(HERE, "results.txt"), "w") as f:
        f.write(f"=== ESNFeedback | ett {FILE} horizon={HORIZON} ===\n")
    ...
    ax.set_ylabel("Standardized OT")
    ax.set_title(f"ESNFeedback — ETT {FILE} {HORIZON}-step forecast (test NMSE={test_score:.4f})")
    ax.legend()
    plt.tight_layout()
    ...
    ax.set_aspect("equal")
    ax.set_title("ESNFeedback — Eigenvalues of W")
    ax.set_xlabel("Re")
    ax.set_ylabel("Im")
    plt.tight_layout()
