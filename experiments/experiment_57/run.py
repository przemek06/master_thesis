...
for run_idx in range(N_RUNS):
    ...
    print("  Optimizing ESNFeedback (feedback disabled)...", flush=True)
    ...
    print(f"  ESN         test mean: {np.mean(test_scores):.4f}, median: {np.median(test_scores):.4f}", flush=True)
    print(f"  persistence test mean: {np.mean(baseline_scores):.4f}, median: {np.median(baseline_scores):.4f}", flush=True)
...
results = {
    "config": {
        "model": "ESNFeedback (feedback_scaling=0)",
...
with open(os.path.join(HERE, "results.txt"), "w") as f:
    f.write("=== Summary ===\n")
    f.write(f"ESN         test mean of means: {run_means.mean():.4f} +/- {run_means.std():.4f}\n")
    f.write(f"ESN         test median of medians: {np.median(run_medians):.4f}\n")
...
model = stored_models[median_run]
test_list = stored_test_lists[median_run]
scores = run_results[median_run]["test"]["scores"]
median_idx = int(np.argsort(np.array(scores))[len(scores) // 2])
...
START = 50
SHOW = 24 * 14
t = np.arange(WARMUP + START, WARMUP + START + SHOW)
fig, ax = plt.subplots(figsize=(14, 4))
ax.plot(t, y_med[WARMUP + START:WARMUP + START + SHOW], label="Target", color="steelblue")
ax.plot(t, pred_med[START:START + SHOW], label="Prediction", color="tomato", linestyle="--", alpha=0.8)
ax.set_xlabel("Hour")
ax.set_ylabel("Standardized load")
ax.set_title(f"ESN — Electricity {HORIZON}h ahead (median run, median test client)\n"
             f"run test mean={run_means[median_run]:.4f}, client NMSE={scores[median_idx]:.4f}")
ax.legend()
plt.tight_layout()
...
ax.bar(range(N_RUNS), run_means, color="steelblue", label="ESN")
ax.plot(range(N_RUNS), base_means, color="black", marker="o", linestyle="--", linewidth=1.5, label="Persistence")
ax.axhline(run_means.mean(), color="red", linestyle="--", linewidth=1.5, label=f"ESN mean = {run_means.mean():.4f}")
ax.set_xlabel("Run index")
ax.set_ylabel("Mean test NMSE")
ax.set_title("ESN — Mean test NMSE per run")
ax.legend()
plt.tight_layout()
...
for arr, label, color in [(all_scores, "ESN", "steelblue"), (all_base, "Persistence", "black")]:
